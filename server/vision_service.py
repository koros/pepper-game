...
class VisionService:
    def __init__(
        ...
        logger.info(
            "Initializing vision service: pc_camera_index=%s preview_enabled=%s live_preview_enabled=%s live_analysis_interval=%s window_name=%s",
            pc_camera_index,
            preview_enabled,
    ...
    def preview_header_text(self, source: str, width: int, height: int) -> str:
        if source.startswith("pepper"):
            return (
                "%s %sx%s | cam=%s res=%s fps=%s color=%s"
                % (
                    source,
                    width,
                    height,
        ...
        return "%s %sx%s" % (source, width, height)
